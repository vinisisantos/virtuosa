...
export type EvaluationConfirmationContext = {
  ...
  prepareConversation?: () => Promise<EvaluationConfirmationContext["conversation"]>;
};
...
export async function resolveEvaluationConfirmationContext(
  req: Request,
...
type ContextResolver = () => Promise<EvaluationConfirmationContext | NextResponse | null>;

export async function getEvaluationConfirmation(resolve: ContextResolver) {
  try {
    ...
    const automation = await findEvaluationConfirmationRequestAutomation(context.unitConfig.unit, prisma, false);
    ...
    return NextResponse.json({
      ...
      windowHours,
      startsConversation: !context.conversation.id,
      instanceName: context.unitConfig.instanceDisplayName,
      phone: context.conversation.contact.phone,
      connected: context.instance.status === "connected",
    });
  } catch (error) {
...
export async function sendEvaluationConfirmation(req: Request, resolve: ContextResolver) {
  ...
  try {
    ...
    // Só materializa a conversa depois da reserva compartilhada com o cron.
    if (context.prepareConversation) {
      context.conversation = await context.prepareConversation();
      triggerData.conversationId = context.conversation.id;
    }
    ...
    return NextResponse.json({
      status: "sent",
      conversationId: context.conversation.id,
      targetInstanceId: context.instance.id,
      appointmentId: context.appointment.id,
      startTime: context.appointment.startTime.toISOString(),
    });
  } catch (error) {
