...
export function ReportsSection({ totalRev, totalCost, balance, sortedProcs, filteredLogs, showClearModal, setShowClearModal, clearAll, selectedMonth, selectedYear, monthlyEvolution, margin, forecastData: initialForecast }:Props) {
  ...
  return (
    <div>
      ...
      {/* Backup & Recovery */}
      <div style={{marginTop:32,...cardS,border:'1px solid rgba(16,185,129,0.15)',background:'rgba(16,185,129,0.02)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:8}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <div style={{width:42,height:42,borderRadius:14,background:'linear-gradient(135deg,#10b981,#059669)',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 4px 12px rgba(16,185,129,0.3)'}}>
              <span className="material-symbols-outlined" style={{fontSize:20,color:'#fff'}}>backup</span>
            </div>
            <div>
              <h3 style={{margin:0,fontSize:'1.05rem',fontWeight:800}}>Backup & Recuperação</h3>
              <p style={{margin:0,fontSize:'0.72rem',color:'#10b981',fontWeight:600}}>
                <span className="material-symbols-outlined" style={{fontSize:12,verticalAlign:'middle'}}>cloud_done</span>
                {' '}Auto-sync ativo — dados salvos no servidor automaticamente
              </p>
            </div>
          </div>
        </div>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          ...
          <button onClick={() => {
            const data = {
              logs: JSON.parse(localStorage.getItem('virtuosa_finance_logs_v2')||'[]'),
              goals: JSON.parse(localStorage.getItem('virtuosa_goals_v3')||'{}'),
              fixed: JSON.parse(localStorage.getItem('virtuosa_fixed_expenses_v2')||'[]'),
              bills: JSON.parse(localStorage.getItem('virtuosa_bills_v2')||'[]'),
            ...
            a.href = url;
            a.download = `virtuosa_backup_${new Date().toISOString().slice(0,10)}.json`;
            a.click();
            URL.revokeObjectURL(url);
          }} style={{
            ...
            color:'#10b981',fontWeight:700,fontSize:'0.82rem',cursor:'pointer',fontFamily:'inherit',
          }}>
          ...
          <button onClick={() => {
            ...
            input.onchange = async (e) => {
              ...
              try {
                ...
                if (!data.logs || !data.goals || !data.fixed || !data.bills) {
                  alert('Arquivo de backup inválido. Certifique-se de usar um arquivo exportado pelo sistema.');
                  return;
                }
                if (!confirm(`Restaurar backup de ${data.exportedAt ? new Date(data.exportedAt).toLocaleString('pt-BR') : 'data desconhecida'}?\n\n⚠️ Isso substituirá TODOS os dados atuais.`)) return;
            ...
            color:'#3b82f6',fontWeight:700,fontSize:'0.82rem',cursor:'pointer',fontFamily:'inherit',
          }}>
          ...
          <button onClick={async () => {
            if (!confirm('Restaurar dados do último backup do servidor?\n\n⚠️ Isso substituirá TODOS os dados locais atuais.')) return;
            try {
              const res = await fetch('/api/backup');
              const data = await res.json();
              if (!data.exists) { alert('Nenhum backup encontrado no servidor.'); return; }
              localStorage.setItem('virtuosa_finance_logs_v2', JSON.stringify(data.logs));
              localStorage.setItem('virtuosa_goals_v3', JSON.stringify(data.goals));
              localStorage.setItem('virtuosa_fixed_expenses_v2', JSON.stringify(data.fixed));
              localStorage.setItem('virtuosa_bills_v2', JSON.stringify(data.bills));
              alert(`✅ Backup do servidor restaurado!\nÚltima atualização: ${new Date(data.updatedAt).toLocaleString('pt-BR')}\n\nA página será recarregada.`);
              window.location.reload();
            } catch {
              alert('❌ Erro ao restaurar do servidor.');
            }
          }} style={{
            display:'flex',alignItems:'center',gap:6,padding:'10px 18px',borderRadius:10,
            border:'1px solid rgba(156,39,176,0.2)',background:'rgba(156,39,176,0.06)',
            color:'#9c27b0',fontWeight:700,fontSize:'0.82rem',cursor:'pointer',fontFamily:'inherit',
          }}>
