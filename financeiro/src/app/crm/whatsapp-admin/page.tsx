...
export default function WhatsAppAdminPage() {
  ...
  const [callBlockSettings, setCallBlockSettings] = useState<CallBlockSettings>({
    enabled: false,
    message: DEFAULT_CALL_BLOCK_MESSAGE,
    cooldownMinutes: 30,
    units: CALL_BLOCK_UNITS,
  });
  const [callBlockLoading, setCallBlockLoading] = useState(true);
  const [callBlockSaving, setCallBlockSaving] = useState(false);
  ...
  const fetchCallBlockSettings = useCallback(async () => {
    setCallBlockLoading(true);
    try {
      const res = await fetch("/api/whatsapp/call-block");
      const data = await res.json();
      if (data.settings) {
        setCallBlockSettings({
          enabled: data.settings.enabled === true,
          message: data.settings.message || DEFAULT_CALL_BLOCK_MESSAGE,
          cooldownMinutes: data.settings.cooldownMinutes || 30,
          units: Array.isArray(data.settings.units) && data.settings.units.length
            ? data.settings.units
            : CALL_BLOCK_UNITS,
        });
      }
      ...
      setCallBlockLoading(false);
    }
  }, []);

  const saveCallBlockSettings = async () => {
    setCallBlockSaving(true);
    try {
      const res = await fetch("/api/whatsapp/call-block", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(callBlockSettings),
      });
      ...
      setCallBlockSettings(data.settings || callBlockSettings);
      const synced = data.webhookSync?.synced || 0;
      const failed = data.webhookSync?.failed || 0;
      toast(
        failed > 0
          ? `Configuração salva. ${synced} webhook(s) atualizados e ${failed} falharam.`
          : `Configuração salva. ${synced} webhook(s) atualizados.`,
        failed > 0 ? "info" : "success",
      );
    } catch {
      toast("Erro ao salvar bloqueio de ligações", "error");
    } finally {
      setCallBlockSaving(false);
    }
  };

  const toggleCallBlockUnit = (unit: string) => {
    setCallBlockSettings((current) => {
  ...
  return (
    <AuthGuard allowedRoles={["ADMINISTRADOR"]}>
      <div className="space-y-6">
        ...
        {/* Bloqueio automático de ligações */}
        <div className="rounded-xl border bg-card text-card-foreground shadow p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${
                      callBlockSettings.enabled
                        ? "bg-emerald-500/10 text-emerald-500"
                        : "bg-muted text-muted-foreground"
                    }`}
                  >
                    {callBlockSettings.enabled ? "Ativo" : "Inativo"}
                  </span>
                </div>
                <p className="mt-1 text-sm text-muted-foreground">
                  Quando uma chamada for detectada, o sistema tenta desligar automaticamente e envia um aviso ao contato.
                </p>
              </div>
            </div>

            <button
              onClick={() => setCallBlockSettings((current) => ({ ...current, enabled: !current.enabled }))}
              disabled={callBlockLoading}
            ...
            >
              {callBlockSettings.enabled ? "Desativar" : "Ativar"}
            </button>
          </div>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_220px]">
            <div>
              ...
              <textarea
                value={callBlockSettings.message}
                onChange={(event) => setCallBlockSettings((current) => ({ ...current, message: event.target.value }))}
                rows={3}
                maxLength={500}
            ...
            <div>
              ...
              <div className="flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-2">
                <input
                  ...
                  min={1}
                  max={1440}
                  value={callBlockSettings.cooldownMinutes}
                  onChange={(event) =>
                    setCallBlockSettings((current) => ({
                      ...current,
          ...
          <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {CALL_BLOCK_UNITS.map((unit) => {
                  const selected = callBlockSettings.units.includes(unit);
                  return (
                    <button
                      key={unit}
                      type="button"
                      onClick={() => toggleCallBlockUnit(unit)}
            ...
            <button
              onClick={saveCallBlockSettings}
              disabled={callBlockSaving || callBlockLoading || callBlockSettings.units.length === 0}
            ...
            >
              {callBlockSaving && <Loader2 className="h-4 w-4 animate-spin" />}
              Salvar bloqueio
            </button>
          </div>
        </div>
