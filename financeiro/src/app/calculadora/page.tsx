...
import { Etapa1, Etapa2, Etapa3, Etapa4, TextField } from './Etapas';
import { FieldHelp } from './FieldHelp';
import { PricingDecision } from './PricingDecision';
...
function Calculator() {
  ...
  return <><AppHeader activePage="calculadora" /><main className="pricing-page">
    <header className="pricing-header"><div><p className="pricing-eyebrow">INTELIGÊNCIA DE PREÇOS</p><h1>Do custo ao preço de venda</h1><p>Construa um preço sustentável e confira como ele se posiciona no mercado.</p></div><button className="pricing-button" onClick={() => void newProtocol()}>Novo cálculo</button></header>
    ...
    {tab === 'calc' ? <>
      {!s.pricing ? <aside className="pricing-notice"><strong>Modelo legado preservado</strong><p>Os percentuais de lucro são acréscimos sobre o custo, não margem sobre a venda. O resultado mantém a fórmula original.</p><button className="pricing-button" onClick={() => void convertLegacy()}>Comparar uma cópia no novo modelo</button></aside> : <section className="pricing-context">
        ...
        <TextField label="Mês de referência dos custos" help="Mês usado como base para despesas e capacidade. Serve para rastrear a origem dos valores. Mudar o mês não busca dados financeiros nem atualiza custos automaticamente." type="month" value={s.pricing.referenceMonth} onChange={referenceMonth => set({ pricing: { ...s.pricing!, referenceMonth } })} />
        <div className="pricing-context-guide"><strong>Como preencher</strong><p>Use os custos do mês de referência. As interrogações explicam cada campo. Taxas e margens iniciais em zero não significam isenção nem recomendação.</p></div>
      </section>}
      ...
      <nav className="pricing-journey" aria-label="Etapas da formação de preço">{[['1', 'Estrutura'], ['2', 'Taxas e margem'], ['3', 'Insumos'], ['4', 'Preço comercial']].map(([n, label]) => <a href={`#pricing-step-${n}`} key={n}><span>{n}</span>{label}</a>)}<a href={s.pricing ? "#pricing-decision" : "#pricing-results"}><span>✓</span>{s.pricing ? "Decisão de venda" : "Resultado legado"}</a></nav>
      <details className="pricing-method"><summary>Como classificar os custos corretamente</summary><div className="pricing-method-grid"><div><strong>Estrutura mensal</strong><p>Aluguel, salários fixos, pró-labore, sistemas e gastos mensais de apoio. São distribuídos pelas horas produtivas planejadas.</p></div><div><strong>Custos por atendimento</strong><p>Insumos consumidos, perdas, profissional e equipamento por sessão. Crescem conforme os atendimentos realizados.</p></div><div><strong>Despesas sobre a venda</strong><p>Impostos, cartão e comissões percentuais incidem sobre o valor cobrado. Desconto reduz o preço; margem é o resultado que se deseja preservar.</p></div></div><p>Um gasto entra uma única vez. Energia e outros custos mistos devem ter sua parcela de estrutura separada do consumo por sessão quando houver essa medição.</p></details>
      ...
      <PricingDecision s={s} set={set} />
      {s.pricing && r.valid && <section className="pricing-panel pricing-installments"><h2>Condição de pagamento para o cliente</h2><p>Total de {fmt(r.precoCobrado)} em {r.parcelas.length} parcela(s). Confira se a taxa informada corresponde a essa condição.</p><div>{r.parcelas.map((v, i) => <span key={i}>{i + 1}ª <strong>{fmt(v)}</strong></span>)}</div></section>}
      ...
      <div className="pricing-bottom"><span>{s.pricing ? 'Cliente paga' : 'Preço legado'} <strong>{price(s.pricing ? r.precoCobrado : r.preco)}</strong></span><button disabled={saving || !r.valid} onClick={() => void handleSave()}>{saving ? 'Salvando…' : 'Salvar protocolo'}</button></div>
