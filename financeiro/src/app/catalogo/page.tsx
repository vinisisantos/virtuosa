      ...
      <main style={{ padding: '24px 32px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        {loading ? <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Carregando...</div> : Object.keys(categories).length === 0 ? (
          <div style={{ ...cardS, textAlign: 'center', padding: 40 }}>
            ...
            <p style={{ color: 'var(--text-muted)', marginTop: 8 }}>Nenhum serviço cadastrado</p>
          </div>
        ) : (
          Object.entries(categories).map(([cat, items]) => (
            <div key={cat} style={{ marginBottom: 20 }}>
              <h3 style={{ margin: '0 0 10px', fontSize: '0.92rem', fontWeight: 900, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: '0.68rem', fontWeight: 600, color: 'var(--text-muted)', padding: '2px 8px', borderRadius: 6, background: 'var(--bg)' }}>{items.length}</span>
              </h3>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }} onClick={() => { setShowModal(false); setEditService(null); }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--card-bg)', borderRadius: 24, padding: 32, maxWidth: 500, width: '100%', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
              ...
              <h2 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 900 }}>{editService ? 'Editar Serviço' : 'Novo Serviço'}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}><label style={labelS}>Descrição</label><input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} style={inputS} placeholder="Descrição do serviço" /></div>
