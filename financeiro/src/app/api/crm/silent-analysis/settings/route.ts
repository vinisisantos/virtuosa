...
import { ensureSilentAnalysisSchema, getSilentAnalysisSettings } from "@/lib/crm-silent-analysis";
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("[Silent Analysis Settings GET]", error);
    return NextResponse.json({ error: "Falha ao carregar configurações da análise silenciosa", details: error?.message }, { status: 500 });
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error("[Silent Analysis Settings PUT]", error);
    return NextResponse.json({ error: "Falha ao salvar configuração da análise silenciosa", details: error?.message }, { status: 500 });
  }
}
