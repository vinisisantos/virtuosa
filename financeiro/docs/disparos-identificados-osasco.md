# Identificação de disparos — Osasco

Escopo aprovado em 09/09/2026: somente novos lotes manuais do Inbox e da Central de Follow-up de Osasco. Não ativa envios, não modifica prazos/cadências/limite de dez, não expande a Central, não marca o histórico por heurística e não modifica o módulo legado de Broadcasts.
...
- A unidade real da instância precisa ser Osasco; em `Todas`, usa a unidade persistida do contato, nunca a unidade declarada no pedido. Em SBC/SCS, o envio mantém seu comportamento e não recebe metadados.
...
- Uma consulta SQL agrupada por atualização da lista de Osasco busca o último disparo de cada conversa visível já autorizada. Busca lateral usa índice parcial por conversa/data/ID, retornando no máximo um registro por conversa, e não varre o histórico completo. Padrão 120 conversas, teto 200. Sem conversa de Osasco, zero consultas adicionais.
- Histórico inclui o JSON na consulta existente, apenas para Osasco. Não há endpoint novo, chamada por selo, consulta ao abrir detalhes ou novo polling. O status mostrado é o último informado pelo WhatsApp nos dados carregados; no chat aberto, o histórico atualizado tem precedência sobre o snapshot da lista.
...
- Homologação operacional: em um envio real autorizado de Osasco, verificar selo no contato, autoria e status; recarregar; verificar mensagem individual sem marcação e ausência em SBC/SCS. Testes do agente não enviam WhatsApp real a leads.
